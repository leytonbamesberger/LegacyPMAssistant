...
// server/http.ts
function redirect(location2, status = 302) {
  return { status, redirect: location2 };
}
...
async function verifyOAuthState(state, config = getProcoreConfig()) {
...
async function exchangeCodeForTokens(code, config = getProcoreConfig()) {
  return postToken(
    {
...
function expiryFromResponse(token) {
  const base = token.created_at ? token.created_at * 1e3 : Date.now();
  return new Date(base + token.expires_in * 1e3).toISOString();
}
async function storeConnection(admin, profileId, token) {
...
async function handleProcoreCallback(query, requestOrigin2) {
  const home = `${resolveAppBaseUrl(requestOrigin2)}/home`;
  if (query.error) {
...
// api/procore/callback.ts
var callback_default = vercelRoute(
  "GET",
  (req) => handleProcoreCallback(
    {
