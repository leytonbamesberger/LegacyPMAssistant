...
// server/procore.ts
async function getConnection(admin, profileId) {
  const { data, error } = await admin.from("procore_connections").select("*").eq("profile_id", profileId).maybeSingle();
  if (error) throw new Error(`getConnection failed: ${error.message}`);
  return data ?? null;
}

// server/procoreRoutes.ts
async function handleProcoreStatus(authorizationHeader) {
...
// api/procore/status.ts
var status_default = vercelRoute(
  "GET",
  (req) => handleProcoreStatus(req.headers.authorization)
);
