...
var require_GoTrueClient = __commonJS({
  "node_modules/@supabase/auth-js/dist/main/GoTrueClient.js"(exports2) {
    ...
    var GoTrueClient = class _GoTrueClient {
      ...
      async fetchJwk(kid, jwks2 = { keys: [] }) {
        let jwk = jwks2.keys.find((key) => key.kid === kid);
        if (jwk) {
          return jwk;
        }
        const now = Date.now();
...
function required(name, ...fallbackNames) {
  ...
  throw new Error(
    `Missing required server env var: ${name}` + (fallbackNames.length ? ` (or ${fallbackNames.join(" / ")})` : "")
  );
}
...
function resolveAppBaseUrl(requestOrigin2) {
  const fromEnv = process.env.APP_BASE_URL;
  if (fromEnv) return fromEnv.replace(/\/$/, "");
  if (requestOrigin2) return requestOrigin2.replace(/\/$/, "");
  return "http://localhost:5173";
}
function isProcoreConfigured() {
  return Boolean(
    process.env.PROCORE_CLIENT_ID && process.env.PROCORE_CLIENT_SECRET && process.env.PROCORE_OAUTH_STATE_SECRET
  );
}
function getProcoreConfig(requestOrigin2) {
  ...
  return {
    ...
    redirectUri: optional(
      ...
      `${resolveAppBaseUrl(requestOrigin2)}/api/procore/callback`
    ),
...
var { get: get3, post, put, head, remove } = defaultApi;
  ...
  */
  async listBuckets(options) {
    ...
    return _this.handleOperation(async () => {
      ...
      return await get3(_this.fetch, `${_this.url}/bucket${queryString}`, { headers: _this.headers });
    });
  }
  /**
  ...
  */
  async getBucket(id) {
    ...
    return _this2.handleOperation(async () => {
      return await get3(_this2.fetch, `${_this2.url}/bucket/${id}`, { headers: _this2.headers });
    });
  }
  /**
  ...
  */
  async listBuckets(options) {
    ...
    return _this2.handleOperation(async () => {
      ...
      return await get3(_this2.fetch, url, { headers: _this2.headers });
    });
  }
  /**
  * @alpha
...
async function verifyMicrosoftToken(authorizationHeader) {
...
function azureOidFromClaims(claims) {
  return claims.oid ?? claims.sub;
}
async function upsertProfile(admin, claims) {
  const azure_oid = azureOidFromClaims(claims);
  const email = claims.email ?? claims.preferred_username ?? "";
  const display_name = claims.name ?? null;
  const { data, error } = await admin.from("profiles").upsert({ azure_oid, email, display_name }, { onConflict: "azure_oid" }).select().single();
  if (error) {
    return {
      error: {
        status: 500,
...
// server/procore.ts
var STATE_TTL = "10m";
var STATE_AUDIENCE = "procore-oauth-state";
function stateKey(config) {
  return new TextEncoder().encode(config.stateSecret);
}
async function signOAuthState(profileId, config = getProcoreConfig()) {
  return new SignJWT({ profileId }).setProtectedHeader({ alg: "HS256" }).setAudience(STATE_AUDIENCE).setIssuedAt().setExpirationTime(STATE_TTL).sign(stateKey(config));
}
function buildAuthorizeUrl(state, config = getProcoreConfig()) {
...
// server/procoreRoutes.ts
async function handleProcoreAuthorize(authorizationHeader, requestOrigin2) {
  ...
  const config = getProcoreConfig(requestOrigin2);
...
function vercelRoute(method, handler) {
...
function requestOrigin(req) {
  ...
  if (!host) return null;
  const proto = firstHeader(req.headers["x-forwarded-proto"]) ?? (host.startsWith("localhost") || host.startsWith("127.0.0.1") ? "http" : "https");
  return `${proto}://${host}`;
}
...
// api/procore/authorize.ts
var authorize_default = vercelRoute(
  "POST",
  (req) => handleProcoreAuthorize(req.headers.authorization, requestOrigin(req))
);
