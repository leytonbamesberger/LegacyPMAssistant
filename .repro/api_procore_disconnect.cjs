...
// api/procore/disconnect.ts
var disconnect_exports = {};
__export(disconnect_exports, {
  default: () => disconnect_default
});
...
// server/procore.ts
async function deleteConnection(admin, profileId) {
  const { error } = await admin.from("procore_connections").delete().eq("profile_id", profileId);
  if (error) throw new Error(`deleteConnection failed: ${error.message}`);
}

// server/procoreRoutes.ts
async function handleProcoreDisconnect(authorizationHeader) {
...
// api/procore/disconnect.ts
var disconnect_default = vercelRoute(
  "POST",
  (req) => handleProcoreDisconnect(req.headers.authorization)
);
