...
import { getAppBaseUrl, isProcoreConfigured } from './config'
...
export async function handleProcoreAuthorize(
  ...
  const state = await signOAuthState(profileResult.profile.id)
  return { status: 200, json: { url: buildAuthorizeUrl(state) } }
}
 ...
 */
export async function handleProcoreCallback(query: {
  code?: string
  state?: string
  error?: string
  error_description?: string
}): Promise<ApiResult> {
  const home = `${getAppBaseUrl()}/home`
  ...
  try {
    ;({ profileId } = await verifyOAuthState(query.state))
  } catch (err) {
